...
public record GetAnticipasQuery(int TenantId, int? BranchId = null, AnticipoStatus? Status = null) : IRequest<IEnumerable<AnticipoDto>>;
    ...
    : IRequestHandler<GetAnticipasQuery, IEnumerable<AnticipoDto>>
{
    ...
    {
        var anticipos = await repo.GetAllByTenantAsync(query.TenantId, query.BranchId, query.Status, ct);
