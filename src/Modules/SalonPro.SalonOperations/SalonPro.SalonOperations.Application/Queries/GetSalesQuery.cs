...
public record GetSalesQuery(
    ...
    DateTime? From = null,
    DateTime? To = null,
    int? BranchId = null) : IRequest<IEnumerable<SaleDto>>;
    ...
    : IRequestHandler<GetSalesQuery, IEnumerable<SaleDto>>
{
    public async Task<IEnumerable<SaleDto>> Handle(GetSalesQuery query, CancellationToken ct)
    {
        ...
        if (query.From.HasValue && query.To.HasValue)
            sales = await repo.GetByTenantAndDateRangeAsync(query.TenantId, query.From.Value, query.To.Value, query.BranchId, ct);
        else
            sales = await repo.GetTodayByTenantAsync(query.TenantId, query.BranchId, ct);
